import os from 'os';
import { expandString, expandServerVars } from '../expand.js';
...
describe('expand module', () => {
  describe('expandString', () => {
    ...
    test('should expand variables embedded in longer strings', () => {
      const { result } = expandString('/home/$USER/.config/$APP/data', { USER: 'chris', APP: 'myapp' }, {});
      expect(result).toBe('/home/chris/.config/myapp/data');
    });

    test('should expand mixed $VAR and ${VAR} in same string', () => {
      const { result } = expandString('$HOME/${APP_DIR}', { HOME: '/users/me', APP_DIR: 'app' }, {});
