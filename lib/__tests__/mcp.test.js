...
describe('mcp module', () => {
  ...
  describe('transformServerForTarget', () => {
    ...
    test('should prefer vars over env for expansion', () => {
      ...
      const result = transformServerForTarget(server, 'cursor', { SHARED: 'from-vars' }, { SHARED: 'from-env' });
  ...
  describe('syncToTargetViaFile', () => {
    ...
    test('should add servers to empty config file', async () => {
      ...
      const mockDeps = {
        ...
        writeFile: async (p, content) => { writtenContent = content; },
        mkdir: async () => {}
      };
    ...
    test('should remove previously managed servers when clean=true', async () => {
      ...
      const result = await syncToTargetViaFile('cursor', targetDef, servers, { clean: true }, previousState, mockDeps);
    ...
    test('should not remove unmanaged servers when clean=true', async () => {
      ...
      const result = await syncToTargetViaFile('cursor', targetDef, servers, { clean: true }, ['context7'], mockDeps);
    ...
    test('should not write in dry run mode', async () => {
      ...
      const result = await syncToTargetViaFile('cursor', targetDef, servers, { dryRun: true }, [], mockDeps);
    ...
    test('should propagate non-ENOENT read errors', async () => {
      ...
      await expect(
        syncToTargetViaFile('cursor', targetDef, [], {}, [], mockDeps)
  ...
  describe('syncToTargetViaCli', () => {
    test('should call claude mcp add-json for new servers', async () => {
      ...
      expect(addCall.args[0]).toBe('mcp');
      expect(addCall.args[1]).toBe('add-json');
      expect(addCall.args[2]).toBe('context7');
      const json = JSON.parse(addCall.args[3]);
      expect(json.command).toBe('npx');
    });

    test('should detect existing servers as unchanged', async () => {
      const mockDeps = {
        execFileSync: (cmd, args) => {
          if (args[1] === 'list') return '  - context7\n';
          return '';
        }
      };
    ...
    test('should remove previously managed servers when clean=true', async () => {
      ...
      expect(removeCall.args).toEqual(['mcp', 'remove', 'old-server']);
    });
    ...
    test('should not make changes in dry run mode', async () => {
      ...
      const mockDeps = {
        execFileSync: (cmd, args) => {
          calls.push({ cmd, args });
          if (args[1] === 'list') return '';
          throw new Error('Should not call anything else');
        }
      };
    ...
    test('should include removals in dry run when clean=true', async () => {
      ...
      const result = await syncToTargetViaCli([], { dryRun: true, clean: true }, ['old-server'], mockDeps);

      expect(result.removed).toEqual(['old-server']);
    });

    test('should handle CLI list failure gracefully', async () => {
      const calls = [];

      const mockDeps = {
        execFileSync: (cmd, args) => {
          calls.push({ cmd, args });
          if (args[1] === 'list') throw new Error('CLI not found');
          return '';
        }
      };
  ...
  describe('syncMcp', () => {
    test('should sync to all enabled targets and update state', async () => {
      ...
      const mockDeps = {
        ...
        loadMcpState: async () => ({}),
        saveMcpState: async (cfg, state) => { Object.assign(savedState, state); },
    ...
    test('should not save state in dry run mode', async () => {
      ...
      const mockDeps = {
        ...
        loadMcpState: async () => ({}),
        saveMcpState: async () => { stateSaved = true; },
        syncToTargetViaFile: async () => ({ added: ['context7'], removed: [], unchanged: [] }),
        syncToTargetViaCli: async () => ({ added: [], removed: [], unchanged: [] })
      };
    ...
    test('should skip unknown targets', async () => {
      ...
      const mockDeps = {
        ...
        loadMcpState: async () => ({}),
        saveMcpState: async () => {},
        syncToTargetViaFile: async () => { throw new Error('Should not be called'); },
        syncToTargetViaCli: async () => { throw new Error('Should not be called'); }
      };
  ...
  describe('checkMcp', () => {
    ...
    test('should detect extra servers to remove', async () => {
      const config = {
        'source-directories': ['/personal'],
        'config-directory': '/merged'
      };
    ...
    test('should handle loadMergedMcpDirectory failure gracefully', async () => {
      ...
      const mockDeps = {
        loadMergedMcpDirectory: async () => { throw new Error('File not found'); },
        getMcpTargets: () => ['cursor'],
        loadMcpState: async () => ({})
      };
    ...
    test('should handle loadMcpState failure gracefully', async () => {
      ...
      const mockDeps = {
        ...
        getMcpTargets: () => ['cursor'],
        loadMcpState: async () => { throw new Error('File error'); }
      };
