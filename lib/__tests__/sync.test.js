...
# Content`;
  ...
  describe('syncToTarget', () => {
    test('should copy skills to target directory', async () => {
      const copiedDirs = [];
      const removedDirs = [];
      const createdDirs = [];

      const mockDeps = {
        mkdir: async (dir) => { createdDirs.push(dir); },
        rm: async (dir) => { removedDirs.push(dir); },
        readdir: async (dir, opts) => {
          if (dir === '/config/skills') {
            return [
        ...
        copyDir: async (src, dest, config) => {
          copiedDirs.push({ src, dest, config });
        },
        loadSkillsDirectory: async () => ({
          skills: [
            { name: 'skill-a', 'disable-model-invocation': true },
            { name: 'skill-b', 'disable-model-invocation': false }
          ]
        })
      };

      await syncToTarget('claude', '/target/claude', '/config', { clean: false }, mockDeps);

      expect(createdDirs).toContain('/target/claude');
      expect(copiedDirs).toHaveLength(2);
      expect(copiedDirs[0].src).toBe(path.join('/config/skills', 'skill-a'));
    ...
    test('should remove orphaned skills when clean=true', async () => {
      ...
      await syncToTarget('claude', '/target/claude', '/config', { clean: true }, mockDeps);
  ...
  describe('syncAll', () => {
    test('should sync to all specified targets', async () => {
      ...
      const mockDeps = {
        syncToTarget: async (name, targetPath, configDir, options) => {
        ...
        loadSkillsDirectory: async () => ({ skills: [] }),
        saveSkillsDirectory: async () => {}
      };

      await syncAll('/config', { targets: ['claude', 'codex'] }, mockDeps);

      expect(syncedTargets).toHaveLength(2);
      expect(syncedTargets[0].name).toBe('claude');
    ...
    test('should update last_sync timestamps', async () => {
      ...
      const mockDeps = {
        ...
        loadSkillsDirectory: async () => ({
          skills: [
            { name: 'skill-a', last_sync: null },
            { name: 'skill-b', last_sync: null }
          ]
        }),
      ...
      await syncAll('/config', { targets: ['claude'] }, mockDeps);

      expect(savedDirectory.skills[0].last_sync).toBeDefined();
      expect(savedDirectory.skills[1].last_sync).toBeDefined();
    });
  });
});
