...
import { zipSkill, zipAllSkills } from '../zip.js';
...
describe('zip module', () => {
  describe('zipSkill', () => {
    test('should create zip with correct structure', async () => {
      ...
      const mockArchive = {
        pipe: (output) => { pipedTo = output; },
        directory: (dir, name) => {
          archivedDirectory = dir;
          archivedName = name;
        },
        finalize: () => { finalizedCalled = true; },
        on: () => {}
      };
    ...
    test('should use override output directory when provided', async () => {
      ...
      const mockDeps = {
        ...
        createArchiver: () => mockArchive,
        access: async () => {},
        mkdir: async (dir) => { createdDir = dir; },
        getSkillsDirectory: () => '/merged/skills',
    ...
    test('should throw error for non-existent skill', async () => {
      ...
      await expect(zipSkill('nonexistent', config, {}, mockDeps))
    ...
    test('should include subdirectories in zip', async () => {
      ...
      const mockArchive = {
        pipe: () => {},
        directory: (dir, name) => { archivedDirectory = dir; },
        finalize: () => {},
        on: () => {}
      };
  ...
  describe('zipAllSkills', () => {
    ...
    test('should handle empty skills directory', async () => {
      ...
      const mockDeps = {
        readdir: async () => [],
        zipSkill: async () => { throw new Error('Should not be called'); },
        getSkillsDirectory: () => '/merged/skills'
      };
