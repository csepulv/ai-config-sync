...
describe('rules module', () => {
  describe('loadRulesState', () => {
    ...
    test('should throw on other read errors', async () => {
      ...
      const config = { 'config-directory': '/merged' };
      await expect(loadRulesState(config, { readFile: mockReadFile }))
  ...
  describe('loadRulesFromSources', () => {
    ...
    test('should handle missing rules directory gracefully', async () => {
      ...
      enoent.code = 'ENOENT';
      const mockReaddir = async () => { throw enoent; };
  ...
  describe('syncRulesToTarget', () => {
    test('should copy new files to target', async () => {
      ...
      const ruleFiles = [
        { filename: 'rules.md', sourcePath: '/source/rules.md' }
      ];

      const result = await syncRulesToTarget('claude-code', '/target/rules', ruleFiles, {}, [], mockDeps);
    ...
    test('should remove previously managed files when clean=true', async () => {
      ...
      const result = await syncRulesToTarget('claude-code', '/target/rules', ruleFiles, { clean: true }, previousState, mockDeps);
    ...
    test('should not remove when clean=false', async () => {
      ...
      const result = await syncRulesToTarget('claude-code', '/target/rules', ruleFiles, {}, previousState, mockDeps);
    ...
    test('should not write in dry run mode', async () => {
      ...
      const result = await syncRulesToTarget('claude-code', '/target/rules', ruleFiles, { dryRun: true }, ['old.md'], mockDeps);
    ...
    test('should not remove in dry run with clean', async () => {
      ...
      const result = await syncRulesToTarget('claude-code', '/target/rules', ruleFiles, { dryRun: true, clean: true }, ['current.md', 'old.md'], mockDeps);
  ...
  describe('syncRules', () => {
    test('should sync rules to all targets and update state', async () => {
      ...
      const mockDeps = {
        ...
        saveRulesState: async (cfg, state) => { Object.assign(savedState, state); }
      };
    ...
    test('should not save state in dry run mode', async () => {
      ...
      const mockDeps = {
        ...
        loadRulesState: async () => ({}),
        saveRulesState: async () => { stateSaved = true; }
      };
