...
export function parseFrontmatter(content) {
  ...
  try {
    ...
    // YAML parse failed (e.g. values contain `: ` without quotes).
    // Quote unquoted values and retry so yaml.dump produces valid output.
    try {
      const fixed = quoteUnquotedValues(match[1]);
      const frontmatter = yaml.load(fixed) || {};
      return { frontmatter, body: match[2] };
    } catch {
      return { frontmatter: {}, body: content };
    }
