...
import { loadSkillsDirectory, loadPluginsDirectory, getTargets, loadConfig } from './config.js';
 ...
 */
export async function checkPlugins(configDir, deps = {}) {
  const {
  ...
  let directory;
  try {
    directory = await loadDir(configDir);
  } catch {
    directory = null;
  }
  ...
  const wanted = directory.plugins.map(p => `${p.name}@${p.marketplace}`);
 ...
 */
export async function checkSkillUpdates(configDir, deps = {}) {
  const {
 ...
 */
export async function checkSkillSync(configDir, deps = {}) {
  const {
    loadSkillsDirectory: loadDir = loadSkillsDirectory,
 ...
 */
export async function checkCatalog(configDir, deps = {}) {
  const {
    getFileMtime: getMtime = getFileMtime,
    readdir = fs.readdir
  } = deps;
  ...
  // Check if skills-directory.yaml is newer
  const directoryPath = path.join(configDir, 'skills-directory.yaml');
 ...
 * Run all checks
 */
export async function runAllChecks(configDir, deps = {}) {
  const {
  ...
  for (const { name, check } of checks) {
    const result = await check(configDir, deps);
    result.name = name;
    results.push(result);
  }
