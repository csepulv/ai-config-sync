...
import { normalizeFrontmatter, updateSourceTimestamps } from './sync.js';
...
export async function copyCustomSkills(config, deps = {}) {
  ...
  for (const skill of merged.skills) {
    ...
    // Check if any file in source is newer than newest file in dest
    try {
      const sourceNewest = await getNewestMtime(sourceSkillDir);
      const destNewest = await getNewestMtime(destSkillDir);
      if (sourceNewest <= destNewest) {
...
async function copyDirectory(src, dest) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      await fs.writeFile(destPath, normalizeFrontmatter(content));
    } else {
