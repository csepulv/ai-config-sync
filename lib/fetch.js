 ...
 * @param {object} [deps] - Optional dependencies for testing
 */
export async function addSkill(url, category, config, deps = {}) {
  const {
    getSourceDirectories: getSrcDirs = getSourceDirectories,
    findSkillSource: findSrc = findSkillSource,
    loadSkillsDirectoryFromSource: loadFromSource = loadSkillsDirectoryFromSource,
    saveSkillsDirectoryToSource: saveToSource = saveSkillsDirectoryToSource,
    fetchSkill: doFetch = fetchSkill,
    getSkillsDirectory: getSkillsDir = getSkillsDirectory
  ...
  // Check if skill already exists in any source
  const existingSource = await findSrc(skillName, config, deps);
  const sourceDirs = getSrcDirs(config);

  // Use existing source or default to first source directory
  const targetSource = existingSource || sourceDirs[0];
  if (!targetSource) {
