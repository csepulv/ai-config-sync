...
export async function loadMergedPluginsDirectory(config, deps = {}) {
  ...
  for (const sourceDir of sourceDirs) {
    ...
    if (directory?.marketplaces) {
      for (const marketplace of directory.marketplaces) {
        if (!seenMarketplaces.has(marketplace.name)) {
          seenMarketplaces.add(marketplace.name);
          mergedMarketplaces.push({ ...marketplace });
        }
      }
    }
  }

  return { plugins: mergedPlugins, marketplaces: mergedMarketplaces };
}
